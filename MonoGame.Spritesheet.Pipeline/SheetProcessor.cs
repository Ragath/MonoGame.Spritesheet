...
public class SheetProcessor : ContentProcessor<TextureContent, SheetContent>
{
    ...
    {
        if (File.Exists(path))
            return JsonSerializer.Deserialize<Dictionary<string, Rectangle>>(File.ReadAllText(path), new JsonSerializerOptions(JsonSerializerDefaults.Web) { IncludeFields = true });
        else
